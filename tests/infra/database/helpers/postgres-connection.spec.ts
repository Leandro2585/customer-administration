...
describe('postgres connection', () => {
	...
	test('should create a new connection', async () => {
		getConnectionManagerSpy.mockReturnValueOnce({ has: jest.fn().mockReturnValue(false) })
		await sut.connect()
	...
	test('should get repository', async () => {
		await sut.connect()
		const repository = sut.getRepository(UserEntity)

		expect(getRepositorySpy).toHaveBeenCalledWith(UserEntity)
	...
	test('should return ConnectionNotFoundError on getRepository if connection is not found', async () => {
		...
		expect(() => sut.getRepository(UserEntity)).toThrow(new ConnectionNotFoundError())
	})
})